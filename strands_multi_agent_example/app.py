...
import computer_science_assistant
import english_assistant
import language_assistant
import math_assistant
import no_expertise
...
1. Analyze incoming student queries and determine the most appropriate specialized agent to handle them:
   ...
   - General Assistant(no_expertise): For all other topics outside these specialized domains
...
3. Decision Protocol:
   ...
   - If query is outside these specialized areas → General Assistant(no_expertise)
...
def get_teacher_agent():
    ...
    if use_general:
        tools.append(no_expertise)
    
    # If no tools selected, default to General Assistant(no_expertise)
    if not tools:
        tools = [no_expertise]
